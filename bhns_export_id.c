...
void sbh_bam_exporting_initial_data(void *vp)
{
  ...
  sbh = sbh_read_physics_from_checkpoint();
  points->grid = sbh->grid;
  
  physics(sbh,ADM_UPDATE_Kij);/* before filling */
  /* fill BH */
  Physics_T *const bh  = init_physics(sbh,BH);
